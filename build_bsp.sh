...
pre_process_armnn_sdk(){
    ...
    git checkout "fd069479e05cd2616a2eacba1c13f99da59bb9a8"
    [ ! -d build ] && mkdir build
    return 0
}
...
compile_install_armnn_sdk_bsp(){
    ...
    #! i.MX 8 系列不支持 OpenCL，因此 ARMCOMPUTECL 应关闭
    cmake .. \
        ...
        # ! https://stackoverflow.com/questions/41964891/yocto-sdk-with-cmake-toolchain-file
    ...
    # -DHALF_INCLUDE=${STAGING_DIR_HOST}${datadir}/arm-compute-library/include \
        ...
        # -DARMCOMPUTE_BUILD_DIR=$ROOT_DIR/arm-computelibrary-imx/build \
        # -DARMCOMPUTE_INCLUDE=${STAGING_DIR_HOST}${datadir}/arm-compute-library \
...
# -----------------------------------------------------------------------------
compile_install_armnn_sdk_bsp       # 使用 BSP 提供库
